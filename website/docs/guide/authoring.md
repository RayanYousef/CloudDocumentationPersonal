...
4. To make the new feature editable in the browser CMS, add a matching collection
   in `static/admin/config.yml` (see the `feature_chatbot` entry as a template) —
   the CMS does not auto-discover new folders.
...
navbar version dropdown as a frozen release. `docs/` itself remains the
in-progress **Latest** version served at the site root (the one the CMS edits).
